var filters = "author/all/topic/all/level/0";
var tags = "tags/all"
var currentPage = 1;
...
function AjaxTask(callback) {
    var fullurl = "api/" + filters + "/" + tags + "/tasks/page/" + currentPage;
    $.ajax({
        type: "GET",
...
$(document).ready(function () {
    ...
    AjaxTask(processTaskData)

    $('select').change(function () {
        var sels = $('select');
        var options = new Array(sels.length);
        for (let i = 0; i < sels.length; i++) {
            options[i] = sels[i].id + "/" + sels[i].value ;
        }
    ...
    $("#TasksList").on('click', 'button', function () {
        ...
        if (str == 't') {
            let tagsName = $(this).attr('name');
            tags = "tags/" + tagsName;
            $('#tokenfield').tokenfield('setTokens', [{ value: tagsName, label: tagsName }]);
            AjaxTask(processTaskData)
        }
    ...
    function TokenChange() {
        tags = "tags/" + $('#tokenfield').tokenfield('getTokensList', ',', false, false);
        AjaxTask(processTaskData);
    }
